...
class FirebaseAuth {
  static async signUp(email, password) {
    const response = await fetch(`${FIREBASE_AUTH_URL}:signUp?key=${API_KEY}`, {
      method: 'POST',
      headers: {
  ...
  static async getUserInfo(idToken) {
    const response = await fetch(`${FIREBASE_AUTH_URL}:lookup?key=${API_KEY}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
class AuthManager {
  static async checkAuthState() {
    return new Promise((resolve) => {
      chrome.storage.local.get(['authToken', 'refreshToken', 'userEmail', 'userId', 'tokenExpiry'], async (result) => {
        ...
        if (result.tokenExpiry && now > result.tokenExpiry) {
          try {
            ...
            const refreshData = await FirebaseAuth.refreshToken(result.refreshToken);
            const newExpiry = now + (parseInt(refreshData.expires_in) * 1000);
  ...
  static async signIn(email, password) {
    try {
      ...
      const expiresIn = parseInt(userData.expiresIn) * 1000;
  ...
  static async signUp(email, password) {
    try {
      const userData = await FirebaseAuth.signUp(email, password);
      const expiresIn = parseInt(userData.expiresIn) * 1000;
      const tokenExpiry = Date.now() + expiresIn;
